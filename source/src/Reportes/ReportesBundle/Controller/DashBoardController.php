...
{
	$diaInicio = "Monday"; //Lunes
	$diaFin = "Sunday"; //Domingo
...
private function dibujarGraficoLineas ($cantidad, $periodo/*, $areas*/)
{
	...
	switch ($periodo)
	{
		...
		$fechas = array_reverse($fechas); // fecha de menor a mayor
		break;
		...
		if (date_format($ayer, 'l') == 'Sunday') //Domingo
		{
		...
		//$hoy = date_create();
		$hoy = date_create('2016-04-13'); //ejemplo
		//$ayer NO IRÍA
		...
		if (date_format($hoy, 'j') == 1)
		{
			...
			for ($i=0; $i < $cantidad; $i++)
			{
				$inicio = date_format($date->modify(date_format($date, 'Y-m-d').' -'.$i.' months'), 'Y-m-d').' 00:00'; // Quita un mes entero con mont
...
private function dameDias ($fechaInicio, $fechaFin) //Toma los dias de inicio a FIN
{
	$dias = [];
