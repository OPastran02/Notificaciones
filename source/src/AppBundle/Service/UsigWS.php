...
class UsigWS{
	...
	private function GET_JSON_OBJECT_FROM_PAGE($URL,$timeout,$json = true)
	{		
		$ch = curl_init();
	...
	public function GET_DIRECCION_NORMALIZADA(&$resultados, $pedido, $calle, $altura)
	{	  
	  $URL = $this->container->getParameter('usig_normalizar_geocodificar')."?calle=" .urlencode($calle). "&altura=" .$altura ."&desambiguar=1";	  
	  ...
	  {	    
	    ...
	    $URL = $this->container->getParameter('usig_convertir_coords')."?x=" .$resultados[$pedido]["x"]. "&y=" .$resultados[$pedido]["y"]. "&output=lonlat";
	    ...
	    $URL = $this->container->getParameter('usig_datos_utiles')."?calle=".urlencode($resultados[$pedido]["Calle"])."&altura=".$resultados[$pedido]["Altura"];
	    $datos = self::GET_JSON_OBJECT_FROM_PAGE($URL,5);
	    ...
	    $URL = $this->container->getParameter('usig_smp')."?cod_calle=".$resultados[$pedido]["CodigoCalle"]."&altura=".$altura;
	    $datos = self::GET_JSON_OBJECT_FROM_PAGE($URL,5,false);
	    $smp = explode('"',$datos);	    	    
	    if($smp[1]=='ErrorPuertaInexistente'){
    	 	$URL = $this->container->getParameter('usig_reversegeocoding')."?x=" .$resultados[$pedido]["x"]. "&y=" .$resultados[$pedido]["y"];
	      	$datos = self::GET_JSON_OBJECT_FROM_PAGE($URL,5,false);
	      	$smp = explode('"',$datos);
	...
	public function normalizarDireccion(&$direccion,&$messageError,&$messageAmbiguedad)
    {        
        ...
        if($direccionNormalizada["normalizado"]["Resultados"] == "OK"){
        	...
        	if(!is_null( $var ) ){
        		$messageError .="No se ha podido guardar la dirección ".$direccion->getCalle()->getCalle()." ".$direccion->getAltura().", Altura Inexistente <br>";
            	return false;
        	}            
        }elseif($direccionNormalizada["normalizado"]["Resultados"] == "Ambiguedad"){
            $messageAmbiguedad .="No se ha podido guardar la dirección por ambiguedad ".$direccion->getCalle()->getCalle()." ".$direccion->getAltura()." <br>";
            return false;
        }else{
    ...
    {
    	$URL = $this->container->getParameter('usig_datos_utiles').'?calle='.urlencode($calle).'&altura='.$altura.'';
